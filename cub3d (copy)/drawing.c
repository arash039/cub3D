...
void	dda_calculation(t_mdata *data)
{
	...
	data->ray.camerax = 2 * data->ray.x / (double)data->rx - 1; //divide by size of screen
...
int		drawing_function(t_mdata *data)
{
	...
	i = data->ray.drawend;
	while (++j < data->ray.drawstart)
		data->main_data.addr[j * data->main_data.line_length / data->main_data.bits_per_pixel +
			data->ray.x] = data->c;
	while (++j <= data->ray.drawend)
	{
		data->main_data.addr[j * data->main_data.line_length / data->main_data.bits_per_pixel +
...
int		mlx_main(t_mdata *data)
{
	...
	screen_size(data);
	data->main_data.img = mlx_new_image(data->main_data.mlx_ptr, data->rx, data->ry);
	...
	data->main_data.mlx_win = mlx_new_window(data->main_data.mlx_ptr, data->rx,
			data->ry, "CUB3D");
